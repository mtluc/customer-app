'use client'
import topShopSlice, { Seller } from '@/store/slices/home/top-shop.Slice'
...
const TopShop = ({ items }: { items: Seller[] }) => {
  useSyncSSR(
    (st) => {
      st.dispatch(topShopSlice.actions.initState(items))
    },
    (st) => {
      st.dispatch(topShopSlice.actions.clear())
    }
  )
