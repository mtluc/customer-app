...
const SearchBar = ({ open, onOpenChanged }: SearchBarProps) => {
  ...
  return (
    <Sheet open={open} onOpenChange={(s) => onOpenChanged(s)}>
      <SheetContent
        side="top"
        className="flex h-screen w-screen flex-col gap-0 bg-gray-500 p-0 shadow-xl !duration-200"
        showBtnClose={false}
      >
        <SheetHeader className="space-y-0 bg-primary py-2">
          <SheetTitle className="hidden" />
          <SheetDescription className="hidden" />
          <div className="m-auto flex w-full max-w-4xl items-center px-2">
            <Button
              variant="ghost"
            ...
            <form action="/auction" className="relative flex-1">
              <Button
                variant="ghost"
                type="submit"
                className="absolute left-2 top-1/2 h-auto -translate-y-1/2 p-0 hover:bg-transparent"
              >
              ...
              <Input
                ...
                name="key"
                className={
                  'rounded-full bg-primary-foreground bg-white stroke-1 pl-10 placeholder:italic ' +
                  (isShowClear ? ' pr-10' : '')
                }
