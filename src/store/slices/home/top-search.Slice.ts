...
export const TopSearchAdapter = createEntityAdapter<TopSearch, string>({
    selectId: (e) => e.id
})

interface TopSearchState extends EntityState<TopSearch, string> {
}

const initialState: TopSearchState = TopSearchAdapter.getInitialState(
    TopSearchAdapter.addMany(
        TopSearchAdapter.getInitialState(),
        [
            {
                id: '20200521080004',
                keyword: 'Đồng hồ lô',
            ...
            },
            {
                id: '20200521080541',
                keyword: 'Loa đài',
                img: '/imgs/top-search/20200521080541.png'
            },
            {
                id: '20221025073516',
                keyword: 'Nikon',
...
export const selectByKeyword = createSelector(
    [(state: TopSearchState) => state, (_, key) => key],
    (s, key) => s.ids.filter(id => !key || s.entities[id].keyword.includes(key)) || []
);
